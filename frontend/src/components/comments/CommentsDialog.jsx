...
const CommentsDialog = ({ open, onOpenChange, postTitle, postId }) => {
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent onPointerDownOutside={(e) => e.preventDefault()} onInteractOutside={(e) => e.preventDefault()} onEscapeKeyDown={(e) => e.preventDefault()} className="glass rounded-2xl w-[calc(100vw-2rem)] max-w-xl h-[85vh] sm:h-[80vh] p-0 gap-0 overflow-hidden !flex !flex-col">
        ...
        <ScrollArea className="flex-1 min-h-0 px-6 py-4">
          {comments.length === 0 ? (<p className="text-sm text-muted-foreground text-center py-8">
              No comments yet. Be the first to comment.
            </p>) : (<ul className="space-y-4">
              {comments.map((c) => (<li key={c.id} className="flex gap-3">
        ...
        <div className="border-t border-border/60 px-6 py-4 space-y-3 bg-background/40 shrink-0">
          <Textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Add a comment…" className="min-h-[72px] resize-none rounded-xl" maxLength={500}/>
          <div className="flex items-center justify-between gap-3">
            <span className="text-xs text-muted-foreground">
              {draft.length}/500
            </span>
            <Button onClick={handleSubmit} size="sm" className="rounded-full bg-gradient-primary text-primary-foreground hover:opacity-90 shadow-glow px-5">
