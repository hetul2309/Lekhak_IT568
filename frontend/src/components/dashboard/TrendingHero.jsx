...
const TrendingHero = ({ posts }) => {
    if (posts.length === 0)
        return null;
    const [hero, ...side] = posts;
    return (<section aria-labelledby="trending-heading" className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <article className="lg:col-span-2 group glass rounded-2xl overflow-hidden shadow-card transition-smooth hover:shadow-glow">
          <div className="relative aspect-[16/9] lg:aspect-[16/8] overflow-hidden">
            <img src={hero.image} alt={hero.title} loading="lazy" className="w-full h-full object-cover transition-smooth group-hover:scale-105"/>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-4">
          {side.slice(0, 3).map((post) => (<article key={post.id} className="group glass rounded-2xl overflow-hidden shadow-card transition-smooth hover:shadow-glow flex sm:flex-col lg:flex-row">
              <div className="relative w-24 sm:w-full lg:w-28 shrink-0 aspect-square sm:aspect-[16/10] lg:aspect-square overflow-hidden">
                <img src={post.image} alt={post.title} loading="lazy" className="w-full h-full object-cover transition-smooth group-hover:scale-105"/>
              </div>
