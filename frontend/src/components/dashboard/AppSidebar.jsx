...
export function AppSidebar() {
    ...
    return (<Sidebar collapsible="icon" className="border-r border-border/60">
      <SidebarHeader className="px-3 py-4">
        <NavLink to="/" className={`flex items-center gap-2 ${collapsed ? "justify-center" : ""}`}>
          <img src={lekhakLogo} alt="Lekhak logo" className="h-9 w-9 rounded-xl object-cover shadow-glow shrink-0"/>
          {!collapsed && (<span className="text-lg font-bold text-gradient-primary">Lekhak</span>)}
        </NavLink>
      </SidebarHeader>
            ...
            <SidebarMenu>
              {items.map((item) => (<SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild tooltip={item.title}>
                    <NavLink to={item.url} end className="hover:bg-accent/60 rounded-lg" activeClassName="bg-accent text-accent-foreground font-medium">
