                    ...
                    {blogs.map((blog) => (
                      <TableRow key={blog.id}>
                        ...
                        <TableCell className="max-w-[260px]">
                          <Link
                            to={`/blog/${blog.id}`}
                            className="text-foreground hover:text-primary transition-colors line-clamp-2 font-medium"
                          >
                            {blog.title}
                          </Link>
                        </TableCell>
...
const VisibilityBadge = ({ visibility }: { visibility: "published" | "draft" }) => {
  if (visibility === "published") {
    return (
      <Badge className="rounded-full bg-success/15 text-success hover:bg-success/20 border border-success/30">
        Published
      </Badge>
    );
  }
  return (
    <Badge className="rounded-full bg-muted text-muted-foreground hover:bg-muted border border-border">
      Draft
    </Badge>
  );
};
